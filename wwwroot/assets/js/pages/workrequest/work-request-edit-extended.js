/**
 * Work Request Edit Page - Extended functionality for Labor/Material and Assets
...
(function ($) {
    ...
    /**
     * Populate labor/materials table from existing work request data
     * @param {Array} laborMaterials - Array of WorkRequestLaborMaterial objects
     */
    window.populateLaborMaterialsFromData = function(laborMaterials) {
        if (!laborMaterials || !Array.isArray(laborMaterials)) return;

        laborMaterials.forEach(item => {
            const laborItem = {
                type: item.isAdhoc ? 'adHoc' : 'jobCode',
                name: item.name,
            ...
            // Add to state
            if (item.isAdhoc) {
                extendedState.laborMaterialItems.adHoc.push(laborItem);
            } else {
                extendedState.laborMaterialItems.jobCode.push(laborItem);
            }

            // Add to table
            addLaborMaterialToTable(laborItem);
        });

        console.log(`Populated ${laborMaterials.length} labor/material items`);
    };

    /**
     * Populate related assets table from existing work request data
     * @param {Array} relatesAssets - Array of WorkRequestRelatedAssetDto objects
     */
    window.populateRelatedAssetsFromData = function(relatesAssets) {
        if (!relatesAssets || !Array.isArray(relatesAssets)) return;

        relatesAssets.forEach(item => {
            const assetItem = {
            ...
            // Add to state
            extendedState.relatedAssets.individual.push(assetItem);

            // Add to table
            addAssetToTable(assetItem);
        });

        console.log(`Populated ${relatesAssets.length} related assets`);
    };
