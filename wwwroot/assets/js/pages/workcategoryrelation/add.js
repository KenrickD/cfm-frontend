/**
 * Work Category Relation - Add Page
 * Handles form submission and dual listbox functionality
 */
...
(function ($) {
    ...
    const CONFIG = {
        apiEndpoints: {
            create: MvcEndpoints.WorkCategoryRelation.Create,
    ...
    function init() {
        ...
        bindEvents();
        loadDropdownData();
        console.log('Work Category Relation add page initialized');
    }
    ...
    function handlePICChange(value) {
        if (!value) {
            dualListbox.setDisabled(true);
            dualListbox.setRightItems([]);
            return;
        }

        dualListbox.setDisabled(false);
    ...
    function loadPropertiesByPIC(idPIC) {
        ...
        //             dualListbox.setRightItems(response.data);
        //         }
        //     },
    ...
    function handleSubmit(e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    showNotification('Work category relation created successfully', 'success');
                    setTimeout(() => {
                        window.location.href = MvcEndpoints.WorkCategoryRelation.Index;
                    }, 1000);
                } else {
                    hideLoadingOverlay();
                    showNotification(response.message || 'Failed to create relation', 'error');
                }
            },
            error: function (xhr, status, error) {
                ...
                let errorMessage = 'An error occurred while creating the relation.';
