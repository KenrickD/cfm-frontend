/**
 * Type Category Settings Page - Configurable Inline Editing with Server-Side Pagination
 * Handles CRUD operations for type-based categories (OtherCategory, OtherCategory2)
 *
 * Configuration via window.CategoryConfig:
 * {
 *   entityName: 'Other Category',
 *   entityNamePlural: 'Other Categories',
 *   endpoints: {
...
(function ($) {
    ...
    const CONFIG = {
        entityName: window.CategoryConfig?.entityName || 'Category',
        entityNamePlural: window.CategoryConfig?.entityNamePlural || 'Categories',
        apiEndpoints: {
            list: window.CategoryConfig?.endpoints?.list || '',
            create: window.CategoryConfig?.endpoints?.create || '',
            update: window.CategoryConfig?.endpoints?.update || '',
            delete: window.CategoryConfig?.endpoints?.delete || ''
        }
    };
    ...
    function saveNewCategory() {
        ...
        // Payload structure matching TypePayloadDto
        $.ajax({
